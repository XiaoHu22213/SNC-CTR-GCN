...
def evaluate(alpha, r1, r2, r3, r4, label):
...
if __name__ == "__main__":
    ...
    num_iterations = 10000  # 设置迭代次数

    print("Starting random search...")
    for _ in tqdm(range(num_iterations), desc="Searching for best alpha"):
        # 随机生成 alpha 值
        alpha = np.round(np.random.uniform(0, 1, 4), 1)

        # 评估当前 alpha
        acc, acc5 = evaluate(alpha, r1, r2, r3, r4, label)
    ...
    for i in tqdm(range(len(label)), desc="Fusing results"):
        ...
        r = r11 * best_alpha[0] + r22 * best_alpha[1] + r33 * best_alpha[2] + r44 * best_alpha[3]
